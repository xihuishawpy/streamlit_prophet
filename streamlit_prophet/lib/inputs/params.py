...
def input_seasonality_params(
    ...
    if add_custom_seasonality := st.checkbox(
        ...
        custom_seasonality: Dict[Any, Any] = {
            "custom_param": {
                "name": st.text_input(
                    "Name",
                    value="custom_seasonality",
                    help=readme["tooltips"]["seasonality_name"],
                )
            }
        }
...
def input_regressors(
    ...
    if len(eligible_cols) > 0:
        ...
        if config_regressors not in ["false", False] and len(
            ...
            st.error("Selected regressors are not in the dataset columns, ")
            st.stop()
